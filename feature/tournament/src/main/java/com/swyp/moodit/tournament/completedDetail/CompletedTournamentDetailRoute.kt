    ...
    LaunchedEffect(Unit) {
        viewModel.sideEffect.collect { sideEffect ->
            when (sideEffect) {
                ...
                is CompletedTournamentDetailContract.SideEffect.NavigateToReportReady -> {
                    navigateToReportReady()
                }
            }
        }
    }
        ...
        MissionDetailLoadingType.NONE -> {
            CompletedTournamentDetailScreen(
                ...
                onSatisfactionShowChange = {
                    viewModel.sendIntent(
                        CompletedTournamentDetailContract.Intent.OnSatisfactionBottomSheetShowChange(
                            it
                        )
                    )
                },
                onFeedbackShowChange = {
                    viewModel.sendIntent(
                        CompletedTournamentDetailContract.Intent.OnFeedbackBottomSheetShowChange(
                            it
                        )
                    )
                },
                onSliderRatingChange = {
                    viewModel.sendIntent(
                        CompletedTournamentDetailContract.Intent.OnSliderRatingChange(
                            it
                        )
                    )
                },
                onToggleFeedbackOption = {
                    viewModel.sendIntent(
                        CompletedTournamentDetailContract.Intent.ToggleFeedbackOption(
                            it
                        )
                    )
                },
                onClearFeedbackOption = { viewModel.sendIntent(CompletedTournamentDetailContract.Intent.ClearFeedbackOption) },
                submitSatisfaction = { viewModel.sendIntent(CompletedTournamentDetailContract.Intent.SubmitSatisfaction) },
                onDeleteDialogShowChange = {
                    viewModel.sendIntent(
                        CompletedTournamentDetailContract.Intent.OnDeleteDialogShowChange(
                            it
                        )
                    )
                },
                onDeleteCompleteDialogShowChange = {
                    viewModel.sendIntent(
                        CompletedTournamentDetailContract.Intent.OnDeleteCompleteDialogShowChange(it)
                    )
                },
                onDeleteClick = { viewModel.sendIntent(CompletedTournamentDetailContract.Intent.OnDeleteClick) },
                onCompleteClick = { viewModel.sendIntent(CompletedTournamentDetailContract.Intent.OnCompleteClick) }
            )
        }
    }
}